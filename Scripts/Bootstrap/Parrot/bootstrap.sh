...
if [ "$1" = "i386" ] || [ "$1" = "amd64" ] ; then
  debootstrap --arch=$1 --variant=minbase --include=systemd,libsystemd0,wget,ca-certificates,busybox-static,gnupg parrot "$TARGET_DIR" http://deb.parrot.sh/parrot/
else
  qemu-debootstrap --arch=$1 --variant=minbase --include=systemd,libsystemd0,wget,ca-certificates,busybox-static,gnupg parrot "$TARGET_DIR" http://deb.parrot.sh/parrot/
fi
...
echo "AnLinux-Parrot" > "$TARGET_DIR/etc/hostname"

COMPONENTS="main contrib non-free non-free-firmware"

echo "deb http://deb.parrot.sh/parrot/ parrot $COMPONENTS" >> "$TARGET_DIR/etc/apt/sources.list"
echo "deb http://deb.parrot.sh/parrot/ parrot-security $COMPONENTS" >> "$TARGET_DIR/etc/apt/sources.list"
echo "deb-src http://deb.parrot.sh/parrot/ parrot $COMPONENTS" >> "$TARGET_DIR/etc/apt/sources.list"

#Import the gpg key correctly
chroot "$TARGET_DIR" wget -qO /etc/apt/trusted.gpg.d/parrot-archive-key.asc https://deb.parrot.sh/parrot/misc/parrotsec.gpg
